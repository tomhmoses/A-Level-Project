#===============================================================================
# Main Processing - Version 23
#
# Changing out the word tile with car parking space
# where tile is still used, tiles will = 2 spaces.
#
...
def tileAndTrim(carParkShape,tiles,spaceQueue):
    ...
    for count in range(len(spaceQueue)): #will now try placing tiles
        ...
        if howManyLeft < 1:
            ...
            messageToPrint = messageToPrint + "Placed only " + str(howManyLeft) + " of " + str(tileLabel) + " spaces sucessfully\n"
...
def GetCarParkPixelLinesToDrawFromPixelShape(carParkShape,pixelsPerMeter,addPerimeterRoad):
    ...
    #print shape.getVerticesCoordinates()
    parkingTilesMade,messageToPrint = tileAndTrim(newShape,tiles,spaceQueue) #a list of tile shapes generated, currently in meters
