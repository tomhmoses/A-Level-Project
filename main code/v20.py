#===============================================================================
# Main Processing - Version 20
#
# optimising the tile and trim to rotate the shape then rotate all back.
# better displaying of spaces
# first attempt at the feedback box.
#
#===============================================================================
...
def tileAndTrim(carParkShape,tiles,listOFTuplesOfNamesOfTilesInListAndHowManyOfEach):
    ...
    for count in range(len(listOFTuplesOfNamesOfTilesInListAndHowManyOfEach)): #will now try placing tiles
        tileLabel = listOFTuplesOfNamesOfTilesInListAndHowManyOfEach[count][0]
        currentTile = tiles[tileLabel]
        if currentTile.getDepth() > currentMaxRowHeight:
        ...
        if howManyLeft < 1:
            #was sucessful
            #print "got to here"
            messageToPrint = messageToPrint + "Placed all " + str(tileLabel) + " spaces sucessfully/n"
            if carParkFull:
                messageToPrint = messageToPrint + "Car park was filled. I filled it up with " + str(-howManyLeft) + " extra " + str(tileLabel) + " spaces."
        else:
            messageToPrint = messageToPrint + "car park was full and ran out of space to place what you wanted"
...
fractionOfLongestLengthWhichIsNotRoad = {'standard':float((sizes['standardD'])/(tiles['standard'].getDepth())), #gets the fraction of the side of the tile to be drawn as the car park space
                                        ...
                                        'motorcycle':float((sizes['motorcycleD'])/(tiles['minibus'].getDepth()))}

def getLinesToDrawFromTiles(tileInstance):
    listOfGroupsOfCoordinatesForLines = []
    if tileInstance.getLabel() in ['standard','minibus','disabled','motorcycle'] and displayJustTiles == False:
...
def GetCarParkPixelLinesToDrawFromPixelShape(shape,pixelsPerMeter):
    ...
    #print shape.getVerticesCoordinates()
    parkingTilesMade,messageToPrint = tileAndTrim(shape,tiles,listOFTuplesOfNamesOfTilesInListAndHowManyOfEach) #a list of tile shapes generated, currently in meters
