...
class imageFrame(Frame):
    def __init__(self,parent,backgroundImageLocation):
        self.myFrame = Tkinter.Frame(parent)
        image = Image.open(backgroundImageLocation) #creates an instance of an image class
        self.canvas = Tkinter.Canvas(self.myFrame, width=image.size[0], height=image.size[1]) #image.size gives you the width and depth
        self.canvas.pack()
        image_tk = ImageTk.PhotoImage(image)
        self.canvas.create_image(image.size[0]//2, image.size[1]//2, image=image_tk) #//2 needed in order to center the image

        self.canvas.bind("<Button-1>", whenImageIsClicked) #<Button-1> is the left mouse button

    def update(self,parent,backgroundImageLocation):
...
def openImageAndGetClickedLocations(window,backgroundImageLocation): #will return clicked locations as a list
...
def createMainWindow(backgroundImageLocation):
    print "==- Creating Main UI"
    global window
    
    window = Tkinter.Tk()
    window.title("Car Parking Simulator") #sets title
    window.config(padx = 5,pady = 5) #sets padding

    window.imageSpace = imageFrame(window,backgroundImageLocation) #adds image
    window.imageSpace.pack()
...
createMainWindow(backgroundImageLocation)
window.mainloop()
#carparkOutline = shape()
#openImageAndGetClickedLocations(window,backgroundImageLocation)
#carparkOutline = shape()
